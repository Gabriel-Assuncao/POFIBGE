...
#'                        labels=TRUE, deflator=TRUE, design=TRUE, savedir=tempdir())
#' survey::svymean(x=~V4104, design=pof.svy2, na.rm=TRUE)
#' pof.svy3 <- get_pof(year=2017, selected=FALSE, anthropometry=TRUE, vars=c("V4104","V4105"),
#'                        labels=TRUE, deflator=TRUE, design=TRUE, savedir=tempdir())
#' survey::svymean(x=~V4104, design=pof.svy3, na.rm=TRUE)}
#' @export

get_pof <- function(year, selected = FALSE, anthropometry = FALSE, vars = NULL,
                     labels = TRUE, deflator = TRUE, design = TRUE, savedir = tempdir())
{
  stop("The get_pof function is under development and will be available soon in package POFIBGE.")
  if (year != 2008 & year != 2017) {
    stop("Year must be equal to 2008 or 2017.")
  }
  ...
  ftpdir <- paste0("ftp://ftp.ibge.gov.br/POF/", year, "/Microdados/")
  ...
  data_pof <- data_pof[data_pof$V0015 == "01",]
  if (anthropometry == TRUE & year == 2017) {
    data_pof <- data_pof[data_pof$W001 == "1",]
    ...
    data_pof <- data_pof[data_pof$M001 == "1",]
    data_pof <- data_pof[,!(names(data_pof) %in% c("V0028", "V00281", "V00282", "V00283", "V0030", "V00301", "V00302", "V00303"))]
    if (selected == FALSE) {
      warning("The selected argument was defined as true for the use of the anthropometry module, since the year is different from 2017.")
    }
  }
  else {
  ...
  if (labels == TRUE) {
    if (exists("pof_labeller", where="package:POFIBGE", mode="function")) {
      ...
      warning("Labeller function is unavailable in package POFIBGE.")
    }
  }
  if (deflator == TRUE) {
    if (exists("pof_deflator", where="package:POFIBGE", mode="function")) {
      ftpdef <- ("ftp://ftp.ibge.gov.br/POF/Documentacao_Geral/")
      ...
      warning("Deflator function is unavailable in package POFIBGE.")
    }
  }
  if (design == TRUE) {
    if (exists("pof_design", where="package:POFIBGE", mode="function")) {
      ...
      warning("Sample design function is unavailable in package POFIBGE.")
    }
  }
  return(data_pof)
}
